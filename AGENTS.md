...
- Config lives in the main `config.json`:
  - `hisense_tvs` is the ordered TV list. The setup UI normally exposes one human-readable name and auto-generates the slug-style internal ID for new TVs; the ID remains available under Advanced identity and must stay stable after Companion buttons reference it. Each TV can use automatic or forced authentication and certificate-profile selection.
  ...
  - `hisense_certificate_profiles` is an ordered list of local certificate/key pairs plus administrator-maintained compatibility notes. Automatic selection tries enabled profiles in order.
  - `hisense_compatible_models` is administrator-maintained confirmed/testing model and firmware text exposed on the setup page and TV API.
...
- Protocol generation can be inferred from the TV's UPnP descriptor. A TV-screen “model is no longer compatible with the current app” pairing warning cannot be observed over MQTT and can indicate an outdated client certificate generation; do not claim TDeck can infer that certificate from IP, model, or firmware alone. Operators select/provide a newer local profile and retry.
- The TV setup page mirrors the Button Templates tree interaction: compact group/root rows, persisted group and TV collapse state, per-TV group dropdown movement, and up/down sibling ordering. Deleting a group moves its TVs to Ungrouped.
- Keep the MAC address field. It is used for Wake-on-LAN power-on and passed to pyvidaa for dynamic credential generation on newer protocol versions; the IP address alone is insufficient for those paths.
