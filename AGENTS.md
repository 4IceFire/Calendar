...
- `/media` requires `page:media`; `/config/atem-media` requires `page:config` and renders a setup-only shell with no library fetches. Action grants `page:media_upload`, `page:media_manage` and `page:media_load` use existing `group_pages` storage, union like other page grants, require library access as well, and default denied for non-admin groups. No separate SQL permission columns are needed.
- Media APIs are browser-session only in this stage, including v1 aliases; existing `atem` or `config` service tokens and the scheduler do not gain media access. Image and thumbnail routes require library access too. Setup needs Config access; executable-path changes through setup additionally require the protected Admin group.
...
- Setup keys are `atem_media_enabled` (false initially), `atem_media_node_path` (blank for PATH discovery), and `atem_media_destinations` (`player`, `label`, `slots` per entry). UI/API numbers are 1-based; only the transport converts to ATEM indexes. Players and slot reservations must be distinct, with at least two exclusive slots per player; device limits and video format come from current hardware state.
...
- Reserved slots must be exclusively used by TDeck. Never upload into any player's retained still selection. ATEM's transfer lock is released before player selection, so do not claim atomic protection against edits by external controllers. No AUX or VideoHub routing is implemented in this stage.
...
- Tests: `python -m unittest discover -s tests -p "test_*.py" -v` includes media library, protected APIs, fake worker and actual child IPC. Node and npm dependencies are required for full transport tests. `tests/media_ui_harness.py` binds 127.0.0.1:5063 with temporary data and blocks hardware traffic; `tests/browser/media.spec.js` refuses writes without its explicit fixture marker. Use a unique `--output` directory when existing OneDrive browser artifacts are locked.
