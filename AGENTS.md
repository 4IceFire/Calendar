...
- `media_routing.py` owns asynchronous output display, recent jobs and the nonblocking VideoHub write reservation. Only `VideohubClient.get_routing_state_strict()` with real port counts/complete routes is suitable for player allocation; never use the shared UI fallback snapshot. Avoid mapped VideoHub inputs feeding other outputs, prefer the target's existing exclusive channel, and confirm still/player selection before writing and verifying the chosen output. Job APIs expose only compact operator fields; diagnostics and captured initiating actor belong in Activity Log.
