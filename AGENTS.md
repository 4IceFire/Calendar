...
- The single `page:media` grant (labelled Media) permits browsing/selecting saved images; display also requires Routing and the initiating user's allowed output/mapped VideoHub input. Optional `page:media_upload` remains in `group_pages` with union semantics but is edited in the lower Media tab, excluded from the top page list. Operator upload requires Media + upload. Config grants library CRUD/read independently; direct player testing requires Config + Media. Legacy `page:media_load`/`page:media_manage` rows are inert and excluded from the registry. No new SQL permission columns.
