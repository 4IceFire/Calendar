...
- Use the existing `MediaRoutingManager` to confirm the image/player and VideoHub output first. Only its success callback may execute ordered saved actions. Hold the active-preset guard through actions, stop on first failure, report partial completion without undo/replay. Device/API `202` may mean accepted rather than hardware-completed. Bound data to 500 presets, 20 actions, 32 KB JSON per action and 64 KB per preset. No external URLs, arbitrary headers or operator-supplied actions.
