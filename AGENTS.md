...
- Config lives in the main `config.json`:
  - `hisense_tvs` is the ordered TV list. Stable IDs are used by groups, API clients, variables, and Companion. Each TV can use automatic or forced authentication and certificate-profile selection.
  - `hisense_tv_groups` is the ordered group list; each group's `tv_ids` is also ordered. A TV may belong to multiple groups.
