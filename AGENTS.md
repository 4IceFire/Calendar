...
- Routing's input list links to `/media?output=N`; the simple picker and separate `/media/upload` keep the output through the flow and return to `/routing` only after confirmed display. `media.html`/`media.js` show images, upload and compact job progress only. Configuration and hardware state polling belong to `media_config.html`/`media_config.js` at `/config/atem-media`, including library/preset management and an optional player test. Operator Routing/Media pages omit shared connection indicators too.
- Action grants `page:media_upload`, `page:media_manage`, `page:media_load` (labelled Media: Display images) retain existing `group_pages` keys/union semantics. Display requires Routing + Media Library + Media display and the initiating user's allowed output/mapped VideoHub input. Upload requires Library + Upload; Config additionally grants library CRUD/read independently of those grants. Direct player testing requires Config + Library + Media display. No new SQL permission columns.
- All media APIs remain browser-session only, including v1 aliases; service tokens and the scheduler are denied. Detailed ATEM state/setup require Config. Image/thumbnail routes allow Library OR Config through explicit `_required_any_page_keys` metadata plus the read wrapper; preserve the central non-API page gate. Setup executable-path changes additionally require protected Admin.
...
- Setup keys are `atem_media_enabled` (false initially), `atem_media_node_path` (blank for PATH discovery), and `atem_media_destinations` (`player`, `label`, `slots`, optional paired `aux`/`videohub_input`). Only fully mapped destinations participate in display; legacy unmapped players remain testable. UI/API numbers are 1-based; only transport converts indexes. Players, AUXes, VideoHub inputs and still reservations must be distinct, with at least two exclusive slots per player; capacities/format/fill-source availability are read from ATEM.
...
- `media_routing.py` owns asynchronous output display, recent jobs and the nonblocking VideoHub write reservation. Only `VideohubClient.get_routing_state_strict()` with real port counts/complete routes is suitable for player allocation; never use the shared UI fallback snapshot. Avoid players feeding other outputs or another AUX, prefer the target's existing exclusive channel, confirm still/player/AUX before writing and verifying the chosen output. Job APIs expose only compact operator fields; diagnostics and captured initiating actor belong in Activity Log.
- The reservation covers normal web/API direct and preset route writes (including token/internal dispatch) plus Config raw player tests; contention returns 409 rather than blocking HTTP. Hardware callbacks are bounded, config/device addresses are captured for the job, and the lock stays held until in-flight writes finish. Complete display deadline is 200s; underlying transfer deadline is 180s. Reserve players/slots/AUXes/VideoHub inputs exclusively to TDeck; no program/keyer/unrelated AUX use. This is not atomic against external controllers/separate CLI processes. Do not automatically replay a failed or interrupted display.
