...
- The login page must not render its hamburger navigation button. When authentication is enabled, signed-out pages must not render or poll the shared integration-status indicators. Keep the login form's standards-based username/current-password autocomplete hints so mobile password managers can recognize it.
...
- Preserve the group editor's top-level page checkboxes and Users/Groups layout. Lower settings use conditional tabs with General always available; hide tabs/panels for ungranted pages without clearing their saved values. Media selection, temporary upload and permanent-save options are subordinate grants inside the Routing tab, excluded from the top page list; Media Library is an independent top-level management page; hide upload options without clearing their value when media selection is off. Keep autosave, scoped selectors, accessible keyboard tabs, mobile overflow handling and Pixie auditorium/device dependencies intact.
...
- Routing's input-step toolbar has a distinct Media action beside Change output linking to `/media?output=N`; the simple picker and separate `/media/upload` keep the output through the flow and return to `/routing` only after confirmed display. `media.html`/`media.js` show images, upload and compact job progress only. Hardware setup and retention settings belong to `media_config.html` at `/config/atem-media`. Image management and the player test belong to `media_library.html` at `/media-library`, linked above Config in the account menu. `media_config.js` initializes either page, only requesting the data its controls need. Routing preset setup is separate at `/config/routing-presets`. The main Routing page retains the standard shared connection indicators and cached status polling. The separate Media/upload/Presets pickers omit those indicators.
- The `page:media` grant is the Allow selecting media option inside Routing and requires `page:routing` for browsing/selecting saved images; display also requires Routing and the initiating user's allowed output/mapped VideoHub input. Optional `page:media_upload` remains in `group_pages` with union semantics but is edited below media selection in the Routing tab, excluded from the top page list. Operator upload requires Routing + Media + upload. Enforce these dependencies in both cached `_User.allows_page` and database `_user_allows_page`, covering pages, APIs, image URLs, landing redirects and View as user. Existing grants combine across groups and remain stored while their parent is off. The independent `page:media_library` grant provides full library management, promotion, upload and raw player tests, without granting Routing or Config. Config retains permanent-image reads for preset configuration and hardware status/setup, but no library mutations. Routing uploads default to temporary; `page:media_save` (requiring Routing + Media + upload) permits permanent uploads. Library managers may also save permanently. No new grant is assigned to existing non-admin groups automatically. Legacy `page:media_load`/`page:media_manage` rows are inert and excluded from the registry. No new SQL permission columns.
- All media APIs remain browser-session only, including v1 aliases; service tokens and the scheduler are denied. Detailed ATEM state permits Config or Media Library; setup requires Config. Image/thumbnail routes allow Routing Media, Media Library or Config through explicit `_required_any_page_keys` metadata plus the read wrapper; preset-only operators may read only images referenced by enabled, individually granted presets with allowed fixed outputs. Preserve the central non-API page gate. Setup executable-path changes additionally require protected Admin.
- Media metadata stores server-attributed `uploaded_by`/`uploaded_by_id`, `created_at`, `temporary` and `expires_at` alongside the existing v1 index. Legacy entries stay permanent with unknown uploader; never invent attribution. Only Library responses expose uploader fields. Default lists and preset editors exclude temporary images; `?collection=temporary` requires Library. Direct temporary image reads/display require Library or the owning authenticated uploader with current Routing upload grants. Reject temporary images in saved routing presets until promoted with PATCH `keep: true`. Promotion retains original attribution/time and clears expiry.
- `media_temporary_retention_days` defaults to 7 (integer 1–365), configured in Media setup and captured at upload; changes affect future uploads. Every 60 seconds, and on library reads/uploads, purge expired local assets under `_media_operation_lock`, skipping active transfers and preset references. Retention never sends hardware commands. Log actual deletion via `media.image.expire`, promotion via `media.image.keep`. Keep index validation, path confinement, atomic mutation and all storage/decoding quotas for temporary and saved uploads together. The multipart contract allows only file/name/temporary/CSRF (four parts); clients cannot submit uploader/expiry values.
