...
| --- | --- | --- |
| `GET /api/media` | Routing Media, Media Library or Config | `{ok, items, permissions}`; each item has `id`, `name`, `preset`, dimensions, size, `created_at`, `temporary`, `expires_at`, `url` and `thumbnail_url`. Defaults to saved images. `?collection=temporary` and uploader fields (`uploaded_by`, `uploaded_by_id`) require Media Library access. |
...
includes player/slot details for Config and Media Library. Saved image URLs require
Routing Media, Media Library or Config and return PNGs with private/no-store caching.
Temporary images are readable/displayable only by their authenticated uploader
with current Routing upload permission or a Media Library manager. They are never
listed in the normal picker and cannot be attached to presets until promoted.
Expired images return 404; background cleanup deletes their local files, leaving
ATEM stills unchanged. Retention changes affect only new uploads. Validation returns 400,
...
are ignored; management requires the independent `page:media_library` grant.
`page:media_save` is a Routing option requiring Media + upload and allows saving
new uploads permanently. Media Library managers can also upload permanently.
