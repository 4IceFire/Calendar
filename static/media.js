function initializeMediaPage() {
  ...
  const setupOnly = root.dataset.setupOnly === 'true';
  let permissions = {};
  ...
  function renderState() {
    ...
    text(el('media-setup-capabilities'), capabilities.players && capabilities.stills ? 'Connected switcher: ' + capabilities.players + ' media players and ' + capabilities.stills + ' still slots. Player and slot numbers start at 1.' : 'Switcher capabilities will appear when connected. Player and slot numbers start at 1.');
    if (el('media-player')) {
  ...
  async function uploadImage(event) {
    ...
    try {
      ...
      renderLibrary();
      renderSelection();
      message('Image uploaded to the library. Select a media player to load it into ATEM.', 'success');
    } catch (error) {
  ...
  function collectDestinations() {
    ...
    Array.prototype.forEach.call(el('media-destinations').children, function(row) {
      ...
      destinations.push({ player: player, label: label || 'Media Player ' + player, slots: numbers });
    });
    return destinations;
  }
  ...
  if (el('media-setup')) {
    ...
    if (setupOnly || window.location.hash === '#media-setup') el('media-setup').open = true;
